...
// ─── Fallback stars (shown if Gaia TAP is unreachable) ───────────────────────
// Real Gaia DR3 data for the 20 brightest stars in the sky.
...
const FALLBACK_STARS = [
  ...
  { source_id: "5561361704129988608", ra: 152.0929, dec: -59.6881, parallax: 37.35,  pmra: -159.47, pmdec: 48.20,   phot_g_mean_mag: 2.25,  bp_rp: 0.13, teff_val: 9700, radial_velocity: -14.2, l: 289.44, b: -4.94 },
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      ...
      // Try live search, fall back to scanning fallback stars by name
      try {
        const results = await searchByName(search);
        if (results.length > 0) return NextResponse.json({ results });
      } catch { /* fall through */ }

      // Fallback: match against our hardcoded bright stars (always works offline)
      const lower = search.toLowerCase();
      const NAMED: Record<string, string> = {
        sirius: "2947050466531873024", canopus: "5853498713190525696",
        arcturus: "4472832130942575872", vega: "2021468729365951744",
        capella: "4043800375492625920", rigel: "3374393697054586240",
    ...
    try {
      ...
      console.warn("[/api/gaia] Gaia TAP unreachable, using fallback stars:", err);
      return NextResponse.json(
